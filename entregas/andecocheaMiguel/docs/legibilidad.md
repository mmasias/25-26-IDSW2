...
| :--- | :--- | :--- | :--- |
| **Nombrado** | (+) | La variable `minutosEnUnDia` al usar final (constante) y un nombre autodescriptivo, el desarrollador no tiene que adivinar qué significa el número. | [Enlace al código](https://github.com/Andecochea/23-24-prg2/blob/main/entregas/AndecocheaMiguel/reto-001/CentroComercial.java) |
| **Nombrado** | (-) 1 | Elementos: `int[] caja` e `int[] objetos`. En la declaración se usa caja y objetos, pero en la lógica de los if y en los System.out usas nItems. Esta inconsistencia crea confusión sobre si se está hablando de la misma entidad o de variables distintas. | [Enlace al PR/Rama](https://github.com/Andecochea/23-24-prg2/pull/1) |
| **Nombrado** | (-) 2 | Elemento: `int caja5`, `nItems5`, `libre5`hay un array para las primeras 4 cajas, pero la quinta caja está definida con variables individuales (caja5, etc.). Esto rompe la estructura lógica del sistema y hace que el nombre no describa. | [Enlace al PR/Rama](https://github.com/Andecochea/23-24-prg2/pull/1) |
| **Comentarios** | (+) | No hay comentarios ya que el codigo se entiendo sin ellos. | [Enlace al código](https://github.com/Andecochea/24-25-PRG1/blob/main/entregas/andecocheaMiguel/Reto-002/PPT.java) |
| **Formato** | (+) | El código respeta la jerarquía de niveles (clase -> método -> bloque). Cada nivel está correctamente desplazado, lo que hace visible el ámbito de las variables, cumpliendo con el principio de jerarquía visual. | [Enlace al código](https://github.com/Andecochea/24-25-EDA1/blob/main/entregas/andecocheaMiguel/reto-001/Fila.java) |
| **Formato** | (-) 1 | Espacios en blanco excesivos: Existen múltiples líneas en blanco consecutivas y bloques de "aire" injustificados entre métodos. Viola el principio de "Líneas cortas y grupos lógicos", ya que dispersa el código innecesariamente y obliga a hacer scroll excesivo. | [Enlace al PR/Rama](https://github.com/Andecochea/24-25-EDA1/pull/1) |
| **Formato** | (-) 2 | Proximidad Vertical: La variable Random random se declara al inicio del método simularAcciones, pero hay lógica antes de su uso intensivo.Las variables deben declararse "tan cerca como sea posible de su utilización" para minimizar su intervalo de vida. | [Enlace al PR/Rama](https://github.com/Andecochea/24-25-EDA1/pull/1) |
| **Estándares** | (+) | Declaración de Atributos: Los atributos `capacidad`, `personas` y `contador` están declarados al inicio de la clase y son private. Esto sigue el estándar de encapsulamiento | [Enlace al código](https://github.com/Andecochea/24-25-EDA1/blob/main/entregas/andecocheaMiguel/reto-001/Fila.java) |
| **Estándares** | (-) 1 | Posición de Llaves y Espaciado: El estándar de Java dicta que la llave de apertura { debe ir en la misma línea que la declaración del método/condicional. Además, hay espacios antes de los paréntesis en algunos métodos. | [Enlace al PR/Rama](https://github.com/Andecochea/24-25-EDA1/pull/2) |
| **Estándares** | (-) 2 | Nomenclatura de Métodos: El método `mensajeMuchaGente()` describe un estado pero no una acción clara. El estándar de la industria para métodos dicta que deben empezar con verbos | [Enlace al PR/Rama](https://github.com/Andecochea/24-25-EDA1/pull/2) |
| **Consistencia** | (+) | Notificación por Consola: El código mantiene una consistencia sólida en el reporte final de resultados. Utiliza una estructura repetitiva de System.out.println que facilita la lectura de los datos de salida al usuario final. | [Enlace al código](https://github.com/Andecochea/23-24-prg2/blob/main/entregas/AndecocheaMiguel/reto-001/CentroComercial.java) |
| **Consistencia** | (-) 1 | Estructura de almacenamiento de Cajas: Se utilizan arrays para las primeras 4 cajas (`int[] caja`, `boolean[] libre`), pero la caja 5 se gestiona con variables individuales (`int caja5`, `boolean libre5`) | [Enlace al PR/Rama](https://github.com/Andecochea/23-24-prg2/pull/2) |
| **Consistencia** | (-) 2 | Lógica de actualización de estado: Para las cajas 1 a 4, la lógica está anidada en un if-else gigante. Sin embargo, para la caja 5, se añade una condición extra (&& activa). | [Enlace al PR/Rama](https://github.com/Andecochea/23-24-prg2/pull/2) |
| **Código Muerto**| (+) | El código utiliza la variable `tiempo` como control del bucle y la variable `cola` para la lógica de negocio. Ambas tienen una referencia activa y un propósito claro en el cálculo de resultados finales. | [Enlace al código](https://github.com/Andecochea/23-24-prg2/blob/main/entregas/AndecocheaMiguel/reto-001/CentroComercial.java) |
| **Código Muerto**| (-) | El array `int[] objetos` y la variable `numeroDeClientes` se declaran pero nunca se utilizan. | [Enlace al PR/Rama](https://github.com/Andecochea/23-24-prg2/pull/3) |
| **DRY** | (+) | Uso de la variable contador: El sistema mantiene una representación única y autoritativa del tamaño actual de la fila mediante `contador`. Se evita recalcular la longitud del array ocupado en cada método, centralizando el conocimiento del estado de la fila. | [Enlace al código](https://github.com/Andecochea/24-25-EDA1/blob/main/entregas/andecocheaMiguel/reto-001/Fila.java) |
| **DRY** | (-) | Lógica de desplazamiento hacia la izquierda | [Enlace al PR/Rama](https://github.com/Andecochea/24-25-EDA1/pull/3) |
| **YAGNI** | (+) | Variable `totalItems` Se mantiene una representación autoritativa del inventario vendido. Es necesaria para el reporte final solicitado| [Enlace al código](https://github.com/Andecochea/23-24-prg2/blob/main/entregas/AndecocheaMiguel/reto-001/CentroComercial.java) |
| **YAGNI** | (-) | Array objetos: El código ya utiliza `nItems` para controlar los productos en cada caja. El array `objetos` es una estructura duplicada e innecesaria que impone restricciones de memoria y complejidad cognitiva sin una necesidad presente. | [Enlace al PR/Rama](https://github.com/Andecochea/23-24-prg2/pull/4) |
