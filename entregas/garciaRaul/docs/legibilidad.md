...
| Categoría | | Justificación | Enlace |
| :--- | :--- | :--- | :--- |
| **Nombrado** | Ejemplo + | Términos como `titulo`, `artista`, `duracion`, `favorita`, son correctos del dominio de una clase `Cancion`, facilitando su lectura. | [Enlace a código](https://github.com/xImRaul/24-25-EDA1/blob/ada07e89535ab1fbc062ee34cfda70435950e826/entregas/garciaRaul/Reto-004/Cancion.java#L2-L5) |
| | Ejemplo - | Al apuntar los dos `Scanner` a un mismo flujo (`System.in`), no tiene sentido que se nombren `scanner1` y `scanner2`. Además, no tienen un significado propio. | [Enlace PR](https://github.com/ximraul/23-24-prg1/pull/1) |
| | Ejemplo - | Los términos `numero1`, `numero2` y `numero3` no tienen significado claro. Al cambiarlos por `numeroCentenas`, `numeroDecenas` y `numeroUnidades` se entienden de una mejor manera. | [Enlace PR](https://github.com/ximraul/23-24-prg1/pull/3) |
| **Comentarios** | | No tengo comentarios en mi código debido a la insistencia del profesor desde el inicio, por lo que no puedo proporcionar ejemplos de buen o mal uso. | |
| **Formato** | Ejemplo + | El uso de una constante `MODO_DEBUG` para gestionar el modo depuración es un excelente ejemplo de buen formato. | [Enlace a código](https://github.com/xImRaul/24-25-EDA2/blob/731eb39ebec13fc5b304ea5bb1fffd1ba32a4885/entregas/garciaRaul/src/RellenoInundacion.java#L6) |
| | Ejemplo - | Mal uso de declaración de la variable `billetes` al principio del código, aumentando innecesariamente su intervalo de vida. | [Enlace PR](https://github.com/ximraul/23-24-prg1/pull/4) |
| | Ejemplo - | El método `personasEnColaAlFinalizar` se declara al principio del código, alargando su uso innecesariamente. | [Enlace PR](https://github.com/ximraul/23-24-prg2/pull/1) |
| **Estándares** | Ejemplo + | Sigue convenciones de Java: constantes en `MAYUSCULAS`, métodos/variables en `camelCase`, variables finales en `SCREAMING_SNAKE_CASE` y uso consistente de `{}`. | [Enlace a código](https://github.com/xImRaul/23-24-prg2/blob/0fb06d853c62cf43407e861e4ac5869ee92bb36c/entregas/garciaRaul/RETO-001/Centrocomercial.java#L1) |
| | Ejemplo - | Intentar asignar un valor `null` a un tipo primitivo `int`. El estándar aplicado en este caso es simplemente inicializar con el valor `0`. | [Enlace PR](https://github.com/ximraul/23-24-prg1/pull/6) |
| | Ejemplo - | El uso de la variable `añadirClientes` puede ser correcta ya que Java usa caracteres Unicode, pero no es el estándar de Java usar las `ñ`, por lo que es mejor práctica utilizar `agregarClientes`. | [Enlace PR](https://github.com/ximraul/24-25-eda1/pull/2) |
| **Consistencia** | Ejemplo + | Todo el proyecto sigue la misma consistencia lógica y estructural. | [Enlace a código](https://github.com/xImRaul/24-25-EDA1/tree/ada07e89535ab1fbc062ee34cfda70435950e826/entregas/garciaRaul/Reto-005) |
| | Ejemplo - | Si se decide gestionar el `Random` mediante un objeto, debe hacerse igual en todos los métodos en lugar de crear múltiples instancias innecesarias. | [Enlace PR](https://github.com/ximraul/24-25-eda1/pull/1) |
| | Ejemplo - | Al declarar `private NodoVersion raiz`, Java ya le asigna `null`. Escribirlo explícitamente en el constructor es redundante. | [Enlace PR](https://github.com/ximraul/24-25-eda1/pull/3) |
| **Código muerto** | Ejemplo + | Todo el código es funcional. No hay bloques de código sin uso o cuya presencia sea injustificable. | [Enlace a código](https://github.com/xImRaul/24-25-EDA1/blob/ada07e89535ab1fbc062ee34cfda70435950e826/entregas/garciaRaul/Reto-004/Cancion.java) |
| | Ejemplo - | El valor `true` inicial en la variable `validLine` nunca se lee, ya que se sobrescribe inmediatamente dentro del bucle `do-while`. | [Enlace PR](https://github.com/ximraul/24-25-eda1/pull/4) |
| **DRY** | Ejemplo + | Reutilización del nombre del método `abierto` para diferentes contextos (por hora o fecha), evitando nombres como `estaAbiertoPorHora()`. | [Enlace a código](https://github.com/xImRaul/23-24-prg2/blob/0fb06d853c62cf43407e861e4ac5869ee92bb36c/entregas/garciaRaul/RETO-003/Cine.java#L17-L22) |
| | Ejemplo - | Repetición del proceso para cada denominación de billete. Se soluciona usando un array de valores y un bucle para procesar la lógica. | [Enlace PR](https://github.com/ximraul/23-24-prg1/pull/7) |
| **YAGNI** | Ejemplo + | En lugar de crear clases complejas como `Persona` o `ColaDeClientes`, el uso de un simple `int` para contar clientes es perfecto para el contexto. | [Enlace a código](https://github.com/xImRaul/23-24-prg2/blob/0fb06d853c62cf43407e861e4ac5869ee92bb36c/entregas/garciaRaul/RETO-001/Centrocomercial.java#L10) |
| | Ejemplo - | El programa solo necesita mostrar el resultado. Al crear la variable `String mayor_Menor`, se ocupa memoria innecesaria que podría ir directo al `println`. | [Enlace PR](https://github.com/ximraul/23-24-prg1/pull/8) |